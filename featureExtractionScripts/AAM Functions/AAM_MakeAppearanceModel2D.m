...
% (hand) object
ObjectPixels=drawObject(base_points,options.texturesize,TrainingData(1).Lines); 
%uncomment
% ObjectPixels = ones(options.texturesize(1), options.texturesize(2));
% ObjectPixels = logical(ObjectPixels);
...
for i=1:s
    g(:,i)=AAM_Appearance2Vector2D(TrainingData(i).I,TrainingData(i).Vertices, base_points, ObjectPixels,options.texturesize);
end
