        ...
        int nrhs, const mxArray *prhs[] )
{
	...
	if(classdouble)
	{
		...
		Iout=mxGetPr(plhs[0]);
    	for(q=0; q<(Osize[0]*Osize[1]*Osize[2]); q++) { Iout[q]=0; }
    }
	else
	{
		plhs[0] = mxCreateNumericArray(3, Osize, mxSINGLE_CLASS, mxREAL);
		Ioutf=(float *)mxGetData(plhs[0]);
        for(q=0; q<(Osize[0]*Osize[1]*Osize[2]); q++) { Ioutf[q]=0; }
    ...
    for(q=0; q<Tsize[0]; q++)
    {
        ...
        Lambdar[0]=Tinv[6]*boundmin[2]+c1;
        Lambdar[1]=Tinv[7]*boundmin[2]+c2;
        Lambdar[2]=Tinv[8]*boundmin[2]+c3;
        for(k=boundmin[2]; k<=boundmax[2]; k++)
        {
            Lambdat[0]=Tinv[3]*boundmin[1]+Lambdar[0];
            Lambdat[1]=Tinv[4]*boundmin[1]+Lambdar[1];
            Lambdat[2]=Tinv[5]*boundmin[1]+Lambdar[2];
            for(j=boundmin[1]; j<=boundmax[1]; j++)
            {
                Lambda[0]=Tinv[0]*boundmin[0]+Lambdat[0];
                Lambda[1]=Tinv[1]*boundmin[0]+Lambdat[1];
                Lambda[2]=Tinv[2]*boundmin[0]+Lambdat[2];
                for(i=boundmin[0]; i<=boundmax[0]; i++)
                {
                    Lambda[3]=1-Lambda[0]-Lambda[1]-Lambda[2];
                    /* Check if voxel is inside the tetrahedron */
                    if((Lambda[0]>-0.0001)&&(Lambda[0]<1.0001)&&(Lambda[1]>-0.0001)&&(Lambda[1]<1.0001)&&(Lambda[2]>-0.0001)&&(Lambda[2]<1.0001)&&(Lambda[3]>-0.0001)&&(Lambda[3]<1.0001))
                    {
						...
						index=i+j*Osize[0]+k*Osize[0]*Osize[1];
                        if(classdouble)
						{
